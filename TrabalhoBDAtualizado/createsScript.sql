...
CREATE TABLE IF NOT EXISTS `supermercado`.`caixa` (
  ...
  `caiValorFechamento` DOUBLE NULL COMMENT 'ValorFechamento, campo para armazenar o valor de fechamento do caixa',
...
CREATE TABLE IF NOT EXISTS `supermercado`.`item_venda` (
  ...
  `itv_venCodigo` INT NOT NULL,
  INDEX `fk_Item Venda_Produtos1_idx` (`itv_proCodigo` ASC),
  INDEX `fk_item_venda_venda1_idx` (`itv_venCodigo` ASC),
...
CREATE TABLE IF NOT EXISTS `supermercado`.`item_compra` (
  ...
  INDEX `fk_item_compra_compra1_idx` (`itc_comCodigo` ASC),
